	...
	$.fn.blurjs = function (options) {
		...
		return this.each(function () {
			...
			tempImg.onload = function () {
				...
				var position_X = (($glue.offset().left) - ($source.offset().left) - (options.offset.x));
				var position_Y = (($glue.offset().top) - ($source.offset().top) - (options.offset.y));
				
				if ($source.css('background-size') == 'cover')
				{
					var sizeBG;
					
					var fx = $source.outerWidth()/tempImg.width;
  					var fy = $source.outerHeight()/tempImg.height;
  					var min = Math.min(fx, fy);
					height = $source.height() + parseInt($source.css('padding-bottom').split('px')[0])
					if (min == fy) {
						sizeBG = $source.width() + 'px auto'
						var aspect = tempImg.height/tempImg.width;
						position_X = position_Y - (height - $source.width()*aspect)/2
					}
					else {
						sizeBG = 'auto ' + height + 'px'
						var aspect = tempImg.width/tempImg.height;
						position_X = position_X - ($source.width() - height*aspect)/2
					}
					$glue.css({
						'background-size' : sizeBG
					})
				}

				var position = (attachment == 'fixed') ? '' : '-' + position_X + 'px -' + position_Y + 'px';
